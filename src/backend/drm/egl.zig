...
pub const EGL = struct {
    ...
    pub fn init(gbm: *GBM) !EGL {
        ...
        glEGLImageTargetTexture2DOES = @ptrCast(c.eglGetProcAddress("glEGLImageTargetTexture2DOES"));
        const display = c.eglGetDisplay(@ptrCast(gbm.device)) orelse return error.EGLGetDisplayError;
        ...
        const context = c.eglCreateContext(display, config, null, &default_context_attributes[0]) orelse return error.EGLCreateContextFailed;
        const surface = c.eglCreateWindowSurface(display, config, @intFromPtr(gbm.surface), null) orelse return error.EGLCreateWindowSurfaceFailed;
