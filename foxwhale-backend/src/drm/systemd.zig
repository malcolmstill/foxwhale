...
pub const Logind = struct {
    ...
    pub fn init(self: *Logind) !void {
        const session_path = try getSessionPath(self.bus, self.session_id);
    ...
    pub fn open(self: *Logind, path: [*:0]const u8) !i32 {
        ...
        const fd = try takeDevice(self.bus, self.session_path, path);
...
pub fn create() !Logind {
    const session_id = try pidGetSession();
    const bus = try busDefaultSystem();
    const fd = try busGetFd(bus);
...
fn pidGetSession() ![*c]u8 {
    ...
    const session: [*c]u8 = undefined;

    const err = c.sd_pid_get_session(pid, &session);
    if (err < 0) {
...
fn busDefaultSystem() !*c.struct_sd_bus {
    ...
    const err = c.sd_bus_default_system(@ptrCast(&b));
    if (err < 0) {
...
fn getSessionPath(bus: *c.struct_sd_bus, session_id: [*c]u8) ![*c]u8 {
    ...
    var res = c.sd_bus_call_method(bus, "org.freedesktop.login1", "/org/freedesktop/login1", "org.freedesktop.login1.Manager", "GetSession", &err, @ptrCast(&msg), "s", &session_id[0]);
    defer {
...
fn activate(bus: *c.struct_sd_bus, session_path: [256]u8) !void {
    ...
    const res = c.sd_bus_call_method(
        bus,
...
fn releaseDevice(bus: *c.struct_sd_bus, session_path: [256]u8, fd: i32) !i32 {
    ...
    var st: os.Stat = undefined;
    const res = linux.fstat(fd, &st);

    if (res < 0) {
        return error.StatFailed;
    }

    const rs = c.sd_bus_call_method(
        bus,
        "org.freedesktop.login1",
        session_path[0..session_path.len],
        "org.freedesktop.login1.Session",
...
//     __major = @bitCast(@truncate(c_uint, ((__dev & @bitCast(c.dev_t, @as(c_ulong, @as(c_uint, 1048320)))) >> @intCast(@import("std").math.Log2Int(c_ulong), 8))));
